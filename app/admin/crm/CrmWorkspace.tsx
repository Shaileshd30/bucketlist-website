...
export default function CrmWorkspace() {
  ...
  const [activity, setActivity] = useState({ activityType: "CALL", status: "", note: "", nextFollowUpAt: "" });
  ...
  async function saveActivity(event: FormEvent) {
    ...
    setSelectedLead(null); setActivity({ activityType: "CALL", status: "", note: "", nextFollowUpAt: "" }); setMessage("Follow-up saved."); await loadLeads();
  }
  ...
  return <main className={styles.shell}>
    <aside className={styles.sidebar}>
      ...
      <nav><button className={styles.active}>◎ <span>CRM workspace</span></button><Link href="/admin">◇ <span>Trips & bookings</span></Link><Link href="/admin/itinerary-planner">✦ <span>Itinerary planner</span></Link></nav>
    ...
    <section className={styles.workspace}>
      ...
      {tab === "leads" && <>
        ...
        <div className={styles.toolbar}><input aria-label="Search leads" placeholder="Search name, mobile or trip…" value={search} onChange={(e) => setSearch(e.target.value)} /><select value={filter} onChange={(e) => setFilter(e.target.value)}><option value="ALL">All stages</option>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
        <div className={styles.tableCard}><div className={styles.tableHead}><span>Customer</span><span>Interest</span><span>Stage</span><span>Next follow-up</span><span>Owner</span></div>
          {loading ? <div className={styles.empty}>Loading customer pipeline…</div> : visibleLeads.length === 0 ? <div className={styles.empty}>No matching leads yet.</div> : visibleLeads.map((lead) => <button className={styles.leadRow} key={lead.id} onClick={() => { setSelectedLead(lead); setActivity((value) => ({ ...value, status: lead.status })); }}><span className={styles.customer}><b>{lead.customer.full_name.charAt(0)}</b><span><strong>{lead.customer.full_name}</strong><small>{lead.customer.phone || lead.customer.email}</small></span></span><span><strong>{lead.interested_trip || "Trip not selected"}</strong><small>{lead.travel_month || lead.source}</small></span><span><em data-status={lead.status}>{statusLabels[lead.status]}</em></span><span><strong>{lead.next_follow_up_at ? new Date(lead.next_follow_up_at).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" }) : "Not scheduled"}</strong></span><span>{lead.assigned_to || "Unassigned"} →</span></button>)}</div>
      </>}
