...
export async function GET(request:Request,context:Context){const denied=await requireAdmin();if(denied)return denied;const {id}=await context.params;if(!validId(id))return Response.json({error:'Invalid booking.'},{status:400,headers});try{
 ...
 const content=qr.data?validateQuotation(qr.data.content):emptyQuotation;const payment=optionMatchesBooking(content.options,content.selectedOptionId,booking.total)?availablePayment(installments,b.booking_status,content.validUntil):null;
