...
export default function ItineraryPlanner({ admin = false }: { admin?: boolean }) {
  ...
  return <main className={`${s.shell} ${admin ? s.staff : ""}`}>
    <header className={s.header}>
      <Link href={admin ? "/admin" : "/"} className={s.brand} aria-label="Bucketlist Adventure"><Image src="/bucketlist-logo.png" alt="Bucketlist Adventure" width={1780} height={1008} priority /></Link>
      {admin && <nav className={s.staffNav} aria-label="Admin workspaces"><Link href="/admin/crm">Customers & leads</Link><Link href="/admin">Trips & bookings</Link><span aria-current="page">Trip studio</span></nav>}
      <div className={s.preferences}><label><span>When</span><input aria-label="Preferred month" type="month" value={month} onChange={e => setMonth(e.target.value)} disabled={busy} /></label><label><span>Travellers</span><input aria-label="Travellers" type="number" min={1} max={100} value={travellers} onChange={e => setTravellers(Math.max(1, Math.min(100, Math.trunc(Number(e.target.value) || 1))))} disabled={busy} /></label></div><button type="button" className={s.outline} onClick={newPlan} disabled={busy}>+ New trip</button>
    </header>
