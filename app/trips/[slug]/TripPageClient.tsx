...
export function TripPageClient({ trip }: { trip: TripData }) {
  ...
  const downloadItinerary = async () => {
    ...
    try {
      const { downloadTripPdf } = await import("@/lib/trip-pdf");
      await downloadTripPdf({ trip, batch: displayBatch, hero: trip.image || primaryImage });
    } catch (error) {
      console.error("Could not generate itinerary PDF:", error);
      alert(error instanceof Error ? error.message : "We could not generate the itinerary PDF right now. Please try again.");
    } finally {
