...
async function getTrips(): Promise<TripData[]> {
  try {
    ...
    return Array.isArray(parsed) && parsed.length > 0 ? parsed : defaultTrips;
  } catch {
    return defaultTrips;
  }
}
...
export async function generateMetadata({
  ...
  const trip = (await getTrips()).find((item) => item.slug === slug) ?? defaultTrips[0];
  ...
  return {
    ...
    openGraph: {
      ...
      description: trip.summary,
      url: `/trips/${trip.slug}`,
      images: [{ url: trip.image, alt: trip.title }],
      type: "article",
    },
  };
}
...
export default async function TripPage({
  ...
  const trip = trips.find((item) => item.slug === slug) ?? trips[0] ?? defaultTrips[0];
