...
export type PdfDesign='editorial'|'classic';
export type QuotationContent={pdfDesign:PdfDesign;intro:string;route:string;hotels:string;itinerary:string;includes:string;excludes:string;terms:string;pricingNote:string;validUntil:string;cover:string;options:PackageOption[];selectedOptionId:string};
export const emptyQuotation:QuotationContent={pdfDesign:'editorial',intro:'A journey thoughtfully planned around you. We look forward to helping you create wonderful memories.',route:'',hotels:'',itinerary:'',includes:'',excludes:'',terms:'Subject to availability at the time of confirmation. Please review the itinerary, inclusions and payment schedule before booking.',pricingNote:'',validUntil:'',cover:'',options:[],selectedOptionId:''};
...
export function availablePayment(rows:QuoteInstallment[],bookingStatus:string,validUntil:string,now=Date.now()){
 if(!Number.isFinite(Date.parse(validUntil+'T23:59:59+05:30')))return null;
...
export function validateQuotation(input:unknown):QuotationContent{
 ...
 if(d.pdfDesign!==undefined&&!['editorial','classic'].includes(d.pdfDesign as string))throw Error('Choose a valid PDF design.');
 result.pdfDesign=d.pdfDesign==='classic'?'classic':'editorial';
...
export function editableQuotation(input:unknown):QuotationContent {
 ...
 for(const key of Object.keys(emptyQuotation) as (keyof QuotationContent)[]){
  if(key==='options'||key==='pdfDesign')continue;
  if(typeof source[key]==='string')result[key]=source[key] as string;
 }
 result.pdfDesign=source.pdfDesign==='classic'?'classic':'editorial';
